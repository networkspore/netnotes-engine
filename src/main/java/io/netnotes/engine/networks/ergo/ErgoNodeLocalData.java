...
public class ErgoNodeLocalData extends ErgoNodeData {
    ...
    public void reset(){
        ...
        try {
            getNetworksData().removeAppResource(m_appFileName);
        } catch (IOException e) {
            try {
                    ...
                    try {
                        ...
                        if(proc.isAlive()){
                            Utils.returnObject(null,getNetworksData().getExecService(), (onSucceeded)->{
                                try {
                                    getNetworksData().addAppResource(m_appFileName);
                                } catch (IOException e) {
                                    try {
                                        Files.writeString(AppConstants.LOG_FILE.toPath(), "Error adding to app resources: " + e.toString(), StandardOpenOption.CREATE, StandardOpenOption.APPEND);
                                    } catch (IOException e1) {
