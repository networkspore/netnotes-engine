...
import io.netnotes.engine.apps.RangeStyle;
...
public class ErgoDexChartView {
    ...
    private static Color m_highlightGreen = Drawing.POSITIVE_HIGHLIGHT_COLOR;
    private static Color m_baseRed = Drawing.NEGATIVE_COLOR;
    private static Color m_highlightRed = Drawing.NEGATIVE_HIGHLIGHT_COLOR;
    ...
    public void addMsgListener(NoteMsgInterface item) {
        if (!m_msgListeners.contains(item)) {
            ...
            /*if(m_marketData.getQuoteSymbol().equals("SigUSD")){
                try {
                    Files.writeString(AppConstants.LOG_FILE.toPath(),"added item:" + item.getId(), StandardOpenOption.CREATE,StandardOpenOption.APPEND);
                } catch (IOException e) {
    ...
    public NoteMsgInterface getListener(String id) {
    ...
    public void setConnectionStatus(int status){
        ...
        if(prevStatus != NoteConstants.STARTED && status == NoteConstants.STARTED){
            start();
        }
    }
    ...
    public void getPoolData(){
        ...
        m_ergoDex.getPoolChart(poolId, currentTime, (onSucceeded)->{
            ...
            if(succededObject != null && succededObject instanceof JsonArray){
                ...
                getDexPriceArray(m_ergoDex.getExecService(), json, (onSpectrumData)->{
                    ...
                    sendMessage(NoteConstants.ERROR, currentTime, poolId, m_errorString);
                });
    ...
    public boolean updateRowChart(ErgoDexNumbers spectrumNumbers, TimeSpan timeSpan, int cellWidth, BufferedImage bImg, int posColor, int negColor) throws ArithmeticException{
        ...
        BigDecimal minValue = BigDecimal.valueOf(0.000000000000000001);
        BigDecimal highValue =  spectrumNumbers.getHigh().max(minValue);
            ...
            try{
                ...
                while(nextSpectrumPrice.get().getTimeStamp() <= epochEnd && nextSpectrumPrice.get().getTimeStamp() > priceData.getEpochStart()){
                    ...
                    try{
                        ...
                        try{
                            Files.writeString(AppConstants.LOG_FILE.toPath(), "Spectrum nextPriceException: " + whileNextException.toString(), StandardOpenOption.CREATE, StandardOpenOption.APPEND);
                ...
                try{
                    Files.writeString(AppConstants.LOG_FILE.toPath(), "Spectrum nextPriceException: " + nextPriceException.toString(), StandardOpenOption.CREATE, StandardOpenOption.APPEND);
                }catch(IOException npioe){
...
//int direction, int RGB1, int RGB2, WritableImage img, PixelReader pR, PixelWriter pW, int x1, int y1, int x2, int y2
        ...
        if (!outOfBounds) {
            g2d.setColor(isLastPositive ? Drawing.POSITIVE_HIGHLIGHT_COLOR : Drawing.NEGATIVE_HIGHLIGHT_COLOR);
            g2d.setFont(m_arial);
