...
public class ErgoDexDataList  {
    ...
    private ErgoDexSort m_sortMethod = new ErgoDexSort();
    ...
    public ErgoDexDataList(ErgoWalletMenu walletMenu, String locationId,Stage appStage, ErgoDex ergoDex, SimpleBooleanProperty isInvert, SimpleDoubleProperty gridWidth, SimpleDoubleProperty gridHeight, TextField updatedField,  SimpleObjectProperty<TimeSpan> timeSpanObject, ScrollPane scrollPane) {
        ...
        m_isInvert.addListener((obs,oldval,newval)->{
            ErgoDexSort sortMethod = getSortMethod();
            sortMethod.setSwapTarget(newval ?  ErgoDexSort.SwapMarket.SWAPPED : ErgoDexSort.SwapMarket.STANDARD);
            sort();
            updateGrid();
    ...
    public void sort(boolean doSave) {
        ...
        switch (type) {
            case ErgoDexSort.SortType.LIQUIDITY_VOL:
                if (isAsc) {
                ...
                break;
            case ErgoDexSort.SortType.LAST_PRICE:
                ...
                break;
            case ErgoDexSort.SortType.BASE_VOL:
                if(!swapped){
                    if (isAsc) {
                ...
                break;
            case ErgoDexSort.SortType.QUOTE_VOL:
                if(!swapped){
                    if (isAsc) {
                        Collections.sort(m_marketsList, Comparator.comparing(ErgoDexMarketItem::getQuoteVolume));
                    } else {
    ...
    public ErgoDexSort getSortMethod(){
        return m_sortMethod;
    }

    public void setSortMethod(ErgoDexSort sortMethod){
        ...
        Utils.getStringFromResource("/ergoDex/contracts/pool/N2TPoolSample.hex",getNetworksData().getExecService(), (onSucceded)->{
            ...
            if(sourceValue != null && sourceValue instanceof String){
                ...
                getUnspentByErgoTree(n2TSampleHex, (onResult)->{
                    ...
                    try {
                        Files.writeString(AppConstants.LOG_FILE.toPath(), "failed: " + onFailed.getSource().getException().toString() + "\n", StandardOpenOption.CREATE, StandardOpenOption.APPEND);
                    } catch (IOException e) {
